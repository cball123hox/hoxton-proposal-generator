...
import { ClosingPackCard } from './ClosingPackCard'
import { ManageClosingPackModal } from './ManageClosingPackModal'
import { getSlideUrl } from '../../../lib/storage'
import type { DbRegion } from '../../../types'

interface ClosingPacksTabProps {
  regions: DbRegion[]
  userId: string
  onRefresh: () => Promise<void>
}

export function ClosingPacksTab({ regions, userId, onRefresh }: ClosingPacksTabProps) {
  ...
  return (
    <div>
      ...
      {regions.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 py-16 text-center">
          ...
          <p className="mt-1 text-xs font-body text-gray-400">
            Create regions in the Intro Packs tab first
          </p>
        </div>
      )}
            ...
            >
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <div>
                  <h3 className="font-heading font-semibold text-hoxton-deep">
                    {viewRegion.display_name} — Closing Pack
                  </h3>
                  <p className="text-sm font-body text-gray-400">
                    {viewRegion.closing_slides_count} slides
                  </p>
                </div>
                <button
                  onClick={() => setViewRegion(null)}
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                  {Array.from({ length: viewRegion.closing_slides_count }).map((_, i) => (
                    <div
                      key={i}
                    ...
                    >
                      <div className="aspect-video overflow-hidden bg-gray-50">
                        <SlideThumb
                          src={getSlideUrl(`closing-${viewRegion.id}/Slide${i + 1}.PNG`)}
                          alt={`Slide ${i + 1}`}
      ...
      {/* Manage Closing Pack Modal */}
      {manageRegion && (
        <ManageClosingPackModal
          region={manageRegion}
          userId={userId}
