...
interface ClosingPackCardProps {
  region: DbRegion
  onViewSlides: () => void
  onManage: () => void
}

export function ClosingPackCard({ region, onViewSlides, onManage }: ClosingPackCardProps) {
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6">
      <div className="mb-4 flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm font-body text-hoxton-slate">
            {region.closing_slides_count} slides
          </p>
        </div>
      ...
      <div className="mb-4 flex gap-1.5 overflow-hidden">
        {region.closing_slides_count > 0 ? (
          <>
            {Array.from({ length: Math.min(6, region.closing_slides_count) }).map((_, i) => (
              <div
                key={i}
                className="h-10 w-16 shrink-0 overflow-hidden rounded border border-gray-100 bg-gray-50"
              >
                <SlideThumb
                  src={getSlideUrl(`closing-${region.id}/Slide${i + 1}.PNG`)}
                  alt={`Slide ${i + 1}`}
                  className="h-full w-full"
                />
              </div>
            ))}
            {region.closing_slides_count > 6 && (
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={region.closing_slides_count === 0}
