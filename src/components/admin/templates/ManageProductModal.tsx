...
export function ManageProductModal({ module: mod, regions, userId, onClose, onRefresh }: ManageProductModalProps) {
  ...
  async function handleBulkUpload(files: File[], replaceAll: boolean) {
    ...
    console.log(`[Product Upload] Starting: files=${files.length}, storagePath=${storagePath}, startNumber=${startNumber}, replaceAll=${replaceAll}, moduleId=${mod.id}`)

    const results = await uploadSlides(files, storagePath, startNumber, setUploadProgress)

    const errors = results.filter((r) => r.error)
    if (errors.length > 0) {
      console.error('[Product Upload] Storage errors:', errors)
    }

    const successfulUploads = results.filter((r) => !r.error)
    console.log(`[Product Upload] Results: ${successfulUploads.length} succeeded, ${errors.length} failed`)
    ...
    console.log('[Product Upload] Inserting product_slides records:', JSON.stringify(records))
    const { error: insertError } = await supabase.from('product_slides').insert(records)

    if (insertError) {
      console.error('[Product Upload] DB insert failed:', insertError)
